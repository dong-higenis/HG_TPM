...
bool canInit(void)
{
  ...
  ret = canOpen(_DEF_CAN1, CAN_NORMAL, CAN_CLASSIC, CAN_500K, CAN_500K);  

  if (ret == false)
  {
    logPrintf("[NG] canOpen()\n");
  }
...
void canMain(void)
{
  ...
  if (millis() - prev_time >= CAN_ERR_CHECK_TIME)
  {
    prev_time = millis();
    canUpdate();
...
void cliCan(cli_args_t *args)
{
  ...
  if (args->argc == 1 && args->isStr(0, "close"))
  {
    canClose(_DEF_CAN1);
    ret = true;
  }
  ...
  if (args->argc == 1 && args->isStr(0, "reset"))
  {
    cliPrintf("can reset\n");
    cliPrintf("can close\n");
    cliPrintf("can open - CAN_NORMAL, CAN_CLASSIC, CAN_500K, CAN_500K\n");
