...
static bool canCmdPing(cmd_t *p_cmd);
...
static uint32_t pre_time_ping = 0;
...
static volatile cmd_process_t cmd_obj[] = 
  {
    ...
    {PKT_TYPE_CMD,  "CMD_CAN_PING",       CMD_CAN_PING,       canCmdPing}
  };
...
bool canThreadupdate(void)
{
  ...
  if (is_enable == true)
  {
    ...
    if (millis() - pre_time_ping >= 2000)
    {
      pre_time_ping = millis();
      if (ping_cnt == 0 && is_ping == true)
      {        
        is_ping = false;
...
bool canCmdPing(cmd_t *p_cmd)
{
  if (is_ping == false)
  {        
    is_ping = true;
    logPrintf("[  ] ping started\n");
  }
...
void canHandleTxMessage(can_msg_t *p_msg)
{
  ...
  buf[index++] = stamp.u8Data[0];
  buf[index++] = stamp.u8Data[1];
  buf[index++] = (uint8_t)p_msg->frame;
  buf[index++] = (uint8_t)p_msg->id_type;
  buf[index++] = (uint8_t)p_msg->dlc;
  buf[index++] = (uint8_t)(p_msg->id >> 0);
  buf[index++] = (uint8_t)(p_msg->id >> 8);
  buf[index++] = (uint8_t)(p_msg->id >> 16);
  buf[index++] = (uint8_t)(p_msg->id >> 24);
  buf[index++] = (uint8_t)(p_msg->length);
  for (int i=0; i<p_msg->length; i++)
    buf[index++] = p_msg->data[i];
